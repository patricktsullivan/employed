...
5. MITRE ATT&CK data is extracted from both array and flat formats
"""
...
from sanitizer import (
    ...
    extract_qa_fields,
    debug_sanitization,
    _extract_mitre_attack,
    _get_primary_mitre_field,
)
...
class TestExtractMitreAttack:
    """Tests for the _extract_mitre_attack helper function."""

    def test_extract_new_array_format(self):
        """New mitre_attack array format should be extracted correctly."""
        alert = {
            'mitre_attack': [
                {
                    'pattern_id': 5818,
        ...
        result = _extract_mitre_attack(alert)
        
        assert len(result) == 2
        assert result[0]['tactic'] == 'Persistence'
        assert result[0]['pattern_id'] == 5818
        assert result[1]['tactic'] == 'Execution'
        assert result[1]['technique_id'] == 'T1059'

    def test_extract_fallback_to_flat_fields(self):
        """Should fall back to flat fields when mitre_attack array is missing."""
        alert = {
            'pattern_id': 99999,
        ...
        assert len(result) == 1
        assert result[0]['tactic'] == 'Custom Intelligence'
        assert result[0]['technique_id'] == 'CST0005'
        assert result[0]['pattern_id'] == 99999  # Uses alert-level pattern_id

    def test_extract_empty_mitre_attack_array(self):
        """Empty mitre_attack array should fall back to flat fields."""
        alert = {
            'mitre_attack': [],
        ...
        assert len(result) == 1
        assert result[0]['tactic'] == 'Machine Learning'

    def test_extract_no_mitre_data(self):
        """Alert with no MITRE data should return empty list."""
        alert = {
            'pattern_id': 12345,
            'cmdline': 'test.exe',
        }
    ...
    def test_extract_normalizes_structure(self):
        """All entries should have consistent keys even if source is missing some."""
        alert = {
            'mitre_attack': [
                {'tactic': 'Persistence'},  # Missing other fields
            ]
        }
        ...
        assert result[0]['tactic'] == 'Persistence'
        assert result[0]['technique'] is None


class TestGetPrimaryMitreField:
    """Tests for the _get_primary_mitre_field helper function."""

    def test_get_primary_from_array(self):
        """Should return first entry's field value."""
        mitre_attack = [
            {'tactic': 'Persistence', 'technique': 'Registry Run Keys'},
            {'tactic': 'Execution', 'technique': 'PowerShell'},
        ]
        
        result = _get_primary_mitre_field(mitre_attack, 'tactic', None)
        
        assert result == 'Persistence'

    def test_get_primary_fallback(self):
        """Should return fallback when array is empty."""
        result = _get_primary_mitre_field([], 'tactic', 'Fallback Tactic')
        
        assert result == 'Fallback Tactic'

    def test_get_primary_none_array(self):
        """Should return fallback when array is None."""
        result = _get_primary_mitre_field(None, 'technique', 'Fallback Technique')
        
        assert result == 'Fallback Technique'


class TestExtractQAFieldsMitreArray:
    """Tests for mitre_attack array extraction in extract_qa_fields."""

    def test_extract_includes_mitre_attack_array(self):
        """Extracted fields should include the full mitre_attack array."""
        alert = {
            'id': 'test-123',
            'mitre_attack': [
                {'tactic': 'Persistence', 'technique': 'Registry'},
                {'tactic': 'Execution', 'technique': 'PowerShell'},
            ],
            'device': {},
        }
        ...
        assert 'mitre_attack' in fields
        assert len(fields['mitre_attack']) == 2
        assert fields['mitre_attack'][0]['tactic'] == 'Persistence'

    def test_extract_populates_flat_fields_from_array(self):
        """Flat tactic/technique fields should be populated from first array entry."""
        alert = {
            'id': 'test-123',
            'mitre_attack': [
                {
        ...
        assert fields['tactic'] == 'Persistence'
        assert fields['tactic_id'] == 'TA0003'
        assert fields['technique'] == 'Registry Run Keys'
        assert fields['technique_id'] == 'T1547.001'

    def test_extract_flat_fields_fallback(self):
        """Flat fields should fall back to alert-level values when array is empty."""
        alert = {
            'id': 'test-123',
            'tactic': 'Custom Intelligence',
            'technique_id': 'CST0005',
        ...
        assert fields['tactic'] == 'Custom Intelligence'
        assert fields['technique_id'] == 'CST0005'
        assert len(fields['mitre_attack']) == 1  # Constructed from flat fields