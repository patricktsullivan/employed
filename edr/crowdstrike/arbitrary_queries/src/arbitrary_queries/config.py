...
class Config:
    ...
    cid_registry_path: Path = field(
        default_factory=lambda: Path("./data/cid_registry.json")
    )
...
def _parse_config_dict(data: dict[str, Any]) -> Config:
    ...
    query_defaults = QueryDefaults(
        ...
        poll_interval_seconds=float(
            qd_data.get("poll_interval_seconds", QueryDefaults.poll_interval_seconds)
        ),
        timeout_seconds=float(
            qd_data.get("timeout_seconds", QueryDefaults.timeout_seconds)
        ),
    )
    ...
    concurrency = ConcurrencyConfig(
        max_concurrent_queries=cc_data.get(
            "max_concurrent_queries", ConcurrencyConfig.max_concurrent_queries
        ),
        retry_attempts=cc_data.get("retry_attempts", ConcurrencyConfig.retry_attempts),
        retry_delay_seconds=float(
            cc_data.get("retry_delay_seconds", ConcurrencyConfig.retry_delay_seconds)
        ),
    )

    # Paths
    paths_data = data.get("paths", {})
    cid_registry_path = Path(
        paths_data.get("cid_registry_path", "./data/cid_registry.json")
    )
